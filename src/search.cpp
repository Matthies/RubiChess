...
int chessposition::rootsearch(int alpha, int beta, int *depthptr, int inWindowLast, int maxmoveindex)
{
    ...
    for (int i = 0; i < rootmovelist.length; i++)
    {
        ...
        if (score > alpha)
        {
            ...
            if (score >= beta)
            {
                ...
                tp.addHash(tte, hash, beta, staticeval, HASHBETA, effectiveDepth, (uint16_t)m->code, true);
    ...
    tp.addHash(tte, hash, alpha, staticeval, eval_type, depth, (uint16_t)bestmove, true);
